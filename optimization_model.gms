...
Sets
    t all hours of a year /0*8760/    // 24h per 365 days
    heat_source heat Sources / electricity, gas, mining /;

Scalars
    total_heat_demand       sum of the heat demand throughout the year in KWh /12000/
    btc_price       fixed bitcoin price in Dollars /34500/ // very volatile so fixed price maybe not best modeling approach but good enough starting point
...
Parameters
    ...
    heat_demand(t)     in-home heat demand for each hour of the year in KWh
         ...
         /       electricity     0.36
                 gas             0.083
                 mining          0.36    /; //Mining uses electricity and since all electric heating is 100% efficient we can assume the same price per KWh
...
Variables
    cost    annual cost of the system in Dollars
...
heat_balance(t) ..      heat_demand(t) =g= sum(heat_source, heat_supply(heat_source, t));
