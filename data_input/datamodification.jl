...
data = DataFrame(CSV.File(origin_file, delim=',', select=[:hour, :load]))
...
demand_plot = @df data plot(:hour, :load, title = "heat demand for NUTS-DE11: Stuttgart", xlabel = "Hours of the year [h]", ylabel = "heat load []")
...
# write the reduced and scaled data withput headers to the target destination .csv-file to use as model input
CSV.write(target_file, data, writeheader = false)
